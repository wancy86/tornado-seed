...
class PostDataStreamer:
    ...
    def begin_part(self,headers):
        ...
        filename = headers[0]['params'].get('filename')
        if filename:
            filename = str(uuid.uuid4()).replace('-', '') + '.' + filename
        else:
    ...
    def release_parts(self):
        ...
        for part in self.parts:
            part["tmpfile"].close()
            # if os.path.exists(part["tmpfile"].name):
            #     os.unlink(part["tmpfile"].name)
            # elif os.path.exists(part["tmpfile"].name + '.mp4'):    
            #     os.unlink(part["tmpfile"].name + '.mp4')
    ...
    def examine(self):
        ...
        #     else:
        #         print(filename, part["tmpfile"].name)        
